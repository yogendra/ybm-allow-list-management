...
// export async function deleteAllowList(allowListId){
//   let allowList = await getAllowList(allowListId);
//   if(
//     allowList != null
//     && allowList.info != null
//     && Array.isArray(allowList.info.cluster_ids)
//     && allowList.info.cluster_ids.length > 0){
//       allowList.info.cluster_ids.forEach(async (clusterId) => await removeClusterAllowList(clusterId, allowListId));
//     }
//   const path =`/allow-lists/${allowListId}`;
//   return ybm.delete(path);
// }
// export async function removeClusterAllowList(clusterId, allowListId){
//   console.info (`removeClusterAllowList - clusterId: ${clusterId}, allowListId: ${allowListId}`);
//   let current = await getClusterAllowListIds(clusterId);
//   let filtered = current
//     .filter( id => id != allowListId)
//   return await updateClusterAllowLists(clusterId, filtered);
// }

// export async function addClusterAllowList(clusterId, allowListId){
//   let current = await getClusterAllowListIds(clusterId);
//   current.push(allowListId);
//   current.sort();
//   return await updateClusterAllowLists(clusterId, current);
// }

// export function shouldUpdateAllowList(allowList, cidrOrIpList){
//   console.info (`shouldUpdateAllowList: Checking if allowList(${allowList.spec.name}) requires to be updated with ${cidrOrIpList}`);
//   if(!allowList){
//     console.info (`shouldUpdateAllowList: allowList does not exist, so update must proceed`);
//     return true;
//   }

//   let updateList = cidrOrIpList
//   if(typeof(cidrOrIpList) === "string"){
//     updateList = [cidrOrIpList];
//   }
//   updateList = updateList.sort().uniq();
//   existingList = allowList.spec.allow_list.sort().uniq();
//   return updateList !== existingList;
// }

// function generateAllowListName(cidrOrIpList){
//   let listString = Array.isArray(cidrOrIpList) ? cidrOrIpList.sort().join(","): cidrOrIpList;
//   let hash = generateHash(listString);
//   return `dyn-${hash}`;
// }
...
//     hash.update(data);
//     return hash.digest('hex').substring(0, 59);
// }
